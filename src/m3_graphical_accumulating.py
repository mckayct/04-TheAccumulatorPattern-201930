...
def draw_parallel_lines(n, point, length, window):
    ...
    starting_point = rg.Point(point.x,point.y)
    x = point.x
    y = point.y
    ending_x_point = point.x + length

    for _ in range (n):
        right_end = rg.Point(ending_x_point,y)
        line = rg.Line(starting_point,right_end)
